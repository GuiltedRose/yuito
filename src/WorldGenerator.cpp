...
WorldGeneratior::WorldGeneratior(unsigned int seed) : rng(seed) {}

std::string WorldGeneratior::makeLocationID(const Vec2i& region, int index) {
    std::osringstream oss;
    oss << "r" << region.x << "_" << region.y << "_loc" << index;
    return oss.str();
}

std::string WorldGeneratior::pickRandom(const std::vector<std::string>& list) {
    std::uniform_int_distrabution<int> dist(0, list.size() - 1);
    return list[dist(rng)];
}

std::vector<Location> WorldGeneratior::generateRegion(const Vec2i& regionCoords) {
    const int locationsPerRegion = 5;

    std::vector<Location> locations;
    for(int i = 0; i < locationsPerRegion; ++i) {
        std::string id = makeLocationID(regionCoords, i);
        std::string terrain = pickRandom(terrainType);
...
void WorldGeneratior::linkAdjacentRegions(const Vec2i& regionCoords) {
